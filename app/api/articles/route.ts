...
export async function GET(req: NextRequest) {
  ...
  // Public endpoint for fetching published articles (no auth required)
  if (isPublic) {
    ...
    try {
      const [articles, totalArticles] = await Promise.all([
        prisma.article.findMany({
          ...
          include: {
            author: {
              select: { id: true, name: true, role: true, imageFile: true },
            },
            category: true,
          },
        }),
      ...
      const formattedArticles = articles.map((article) => ({
        ...
        imageFile:
          article.imageFile &&
          typeof article.imageFile === "object" &&
          (article.imageFile as any) instanceof Buffer
        ...
        author: article.author
          ? {
              ...
              image:
                ...
                typeof article.author.imageFile === "object" &&
                (article.author.imageFile as any) instanceof Buffer
  ...
  // Existing authenticated endpoint for admin use
  ...
  if (startDate || endDate) {
    ...
    if (startDate)
      where[dateType === "updated" ? "updatedAt" : "createdAt"].gte = new Date(
        startDate
      );
    if (endDate) {
      const endDateObj = new Date(endDate);
      endDateObj.setHours(23, 59, 59, 999); // Set to end of day
      where[dateType === "updated" ? "updatedAt" : "createdAt"].lte =
        endDateObj;
    }
  }
  ...
  try {
    const [articles, totalArticles] = await Promise.all([
      prisma.article.findMany({
        ...
        orderBy: { createdAt: "desc" },
        include: {
          author: { select: { name: true, role: true, imageFile: true } },
          category: true,
        },
      }),
      prisma.article.count({ where }),

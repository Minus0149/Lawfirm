...
export default function AnalyticsPage() {
  ...
  const fetchArticlesData = async (page: number) => {
    ...
    try {
      const response = await fetch(`/api/articles?${searchParams.toString()}&page=${page}&limit=${articlesPageSize}`)
  ...
  const fetchAdvertisementsData = async (page: number) => {
    ...
    try {
      const response = await fetch(
        `/api/advertisements?${searchParams.toString()}&page=${page}&limit=${advertisementsPageSize}`,
      )
      const data = await response.json()
